for (var i = -4; i < 5; i++) {
	...
	if (_display_index + i >= 1) {
		...
		// Wrap invalid values
		var _battle_index = (_display_index + i) % array_length(global.enemy.title);
		if (_battle_index < 0) _battle_index = array_length(global.enemy.title) + _battle_index;
		
		var _alpha = 1 - abs(320 - _x_center) / (120 * 5);
		draw_set_alpha(_alpha);
		draw_rectangle(_x_center - 50, _y_center - 50, _x_center + 49, _y_center + 49, 0);
		draw_set_alpha(1);
		draw_set_color(c_black);
		draw_rectangle(_x_center - 48, _y_center - 48, _x_center + 47, _y_center + 47, 0);
		draw_set_color(c_white);
		
		var _sprite = asset_get_index($"spr_menu_{global.enemy.name[_battle_index]}");
		if (!sprite_exists(_sprite)) _sprite = spr_menu_empty;
		
		draw_sprite_ext(_sprite, 0, _x_center, _y_center, 96 / sprite_get_width(_sprite), 96 / sprite_get_height(_sprite), 0, c_white, _alpha);
	
		draw_set_halign(fa_middle);
		draw_set_font(font_mars_needs_cunnilingus);
		draw_set_alpha(_alpha);
		draw_text_transformed(_x_center, _y_center + 55, global.enemy.title[_battle_index], 0.5, 0.5, 0);
		draw_set_halign(fa_left);
		draw_set_alpha(1);
		
		var _badges = global.badges[_battle_index];
...
draw_set_halign(fa_middle);
draw_set_font(font_mars_needs_cunnilingus);
draw_text_ext_transformed(x, y + 100, global.enemy.description[_battle_index], 20, 540 / 0.6, 0.6, 0.6, 0);
draw_set_halign(fa_left);