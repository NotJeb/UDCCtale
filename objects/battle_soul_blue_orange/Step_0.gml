...
if (Battle_GetState() == BATTLE_STATE.IN_TURN && moveable)
{
    if (dir == DIR.UP || dir == DIR.DOWN) {
		if (Input_IsHeld(INPUT.LEFT))
			last_input = INPUT.LEFT;
		else if (Input_IsHeld(INPUT.RIGHT))
			last_input = INPUT.RIGHT;
		else if (last_input != INPUT.LEFT && last_input != INPUT.RIGHT)
			last_input = choose(INPUT.LEFT, INPUT.RIGHT);
	}
    if (dir == DIR.LEFT || dir == DIR.RIGHT) {
		if (Input_IsHeld(INPUT.UP))
			last_input = INPUT.UP;
		else if (Input_IsHeld(INPUT.DOWN))
			last_input = INPUT.DOWN;
		else if (last_input != INPUT.UP && last_input != INPUT.DOWN)
			last_input = choose(INPUT.UP, INPUT.DOWN);
	}
    repeat (Input_IsHeld(INPUT.CANCEL) ? Player_GetSpdTotal() * 5 : Player_GetSpdTotal() * 10)
    {
        if (dir == DIR.UP || dir == DIR.DOWN) {
            if (last_input == INPUT.LEFT && !(position_meeting((x - sprite_width / 2), y, block)))
				x -= 0.1;
            if (last_input == INPUT.RIGHT && !(position_meeting((x + sprite_width / 2), y, block)))
				x += 0.1;
        }
        else if (dir == DIR.LEFT || dir == DIR.RIGHT) {
            if (last_input == INPUT.UP && !(position_meeting(x, (y - sprite_height / 2), block)))
				y -= 0.1;
            if (last_input == INPUT.DOWN && !(position_meeting(x, (y + sprite_height / 2), block)))
                y += 0.1;
        }
    }
